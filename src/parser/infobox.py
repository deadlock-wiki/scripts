...
def infobox_hero(hero_data):
    ...
    # Ensure all the hero data is in the parameter_order, used for catching changes to template calls
    # or new hero data not yet handled
    ...
    # Tried for too long to get something that worked in less than O(a*b), maybe im missing something
    # obvious
    infobox_data = {}
    ...
    for key, value in infobox_data.items():
            ...
            for scalingStat in hero_data['SpiritScaling']:
                if key.endswith(scalingStat):
                    if hero_data['SpiritScaling'][scalingStat] == 0.0:
                        break
                    value = (
                        str(value) + ' {{ss|' + str(hero_data['SpiritScaling'][scalingStat]) + '}}'
                    )
                    break
        ...
        # Append the level scaling value to the base value inside {{ls|}} template call
            ...
            ls template still needs to be created; 
            infobox uses a different formatting than this currently
            Search for the scaled stat that ends with the current key
            """
            for scalingStat in hero_data['LevelScaling']:
                if key.endswith(scalingStat):
                    if hero_data['LevelScaling'][scalingStat] == 0.0:
                        break
                    value = (
                        str(value) + ' {{ls|' + str(hero_data['LevelScaling'][scalingStat]) + '}}'
                    )
                    break
        ...
        """
        Grey Talon has rounds per second scaling and fire rate scaling listed; though only base 
        rounds per second is in data.
        The rounds per second scalar is equivalent to the fire rate scalar, but only the fire rate 
        one's intuitive application is correct because the rounds per second one implies that its
        (RPS + RPS from spirit) * Fire rate
        when its really
        RPS * Fire rate + RPS from spirit which is equal to RPS * (Fire rate + Fire rate from spirit)
        Therefore, the scaling should only be displayed for fire rate. There are no other spirit 
        scalars for stats that have final multipliers where this would be replicated.
        """
...
if __name__ == '__main__':
    ...
    hero_data = {
        ...
        'TechDuration': 1,
        'TechRange': 1,
        'SpiritScaling': {'MaxMoveSpeed': 0.04, 'RoundsPerSecond': 0.00625, 'FireRate': 0.25},
        'WeaponPower': 0,
        'WeaponPowerScale': 1,
    }
